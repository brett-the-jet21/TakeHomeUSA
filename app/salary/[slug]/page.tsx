...
export function generateStaticParams() {
  ...
  for (const stateSlug of Object.keys(STATES)) {
    for (let a = 30000; a <= 300000; a += 5000) {
      out.push({ slug: `${a}-salary-after-tax-${stateSlug}` });
    }
  }
  return out;
}
...
export async function generateMetadata({ params }: { params: Params }) {
  ...
  const stateData = STATES[parsed.stateSlug];
            ...
            {relatedSalaries.map((a) => (
              <li key={a}>
                <a
                  ...
                  href={`/salary/${a}-salary-after-tax-${parsed.stateSlug}`}
                >
